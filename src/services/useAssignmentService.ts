...
export const useAssignmentService = () => {
  ...
  const createAssignment = async (variables: CreateAssignmentVariables) => {
    try {
      ...
      const { data, errors } =
        await apolloClient.mutate<CreateAssignmentResponse>({
      ...
      if (errors) {
        ...
        throw new Error(
          errors[0]?.message || "An error occurred during assignment creation"
        );
      }

      if (!data?.createAssignment.success) {
        throw new Error(
          data?.createAssignment.message || "Failed to create assignment"
        );
      }
  ...
  const updateAssignment = async (variables: UpdateAssignmentVariables) => {
    try {
      ...
      const { data, errors } =
        await apolloClient.mutate<UpdateAssignmentResponse>({
      ...
      if (errors) {
        console.error("GraphQL errors:", errors);
        throw new Error(
          errors[0]?.message || "An error occurred during assignment update"
        );
      }
  ...
  const deleteAssignment = async (variables: DeleteAssignmentVariables) => {
    try {
      ...
      const { data, errors } =
        await apolloClient.mutate<DeleteAssignmentResponse>({
      ...
      if (errors) {
        console.error("GraphQL errors:", errors);
        throw new Error(
          errors[0]?.message || "An error occurred during assignment deletion"
        );
      }
  ...
  const createAssignmentQuestion = async (
    variables: CreateAssignmentQuestionVariables
  ) => {
    try {
      setLoading(true);
      setError(null);
      ...
      const { data, errors } =
        await apolloClient.mutate<CreateAssignmentQuestionResponse>({
      ...
      if (errors) {
        console.error("GraphQL errors:", errors);
        throw new Error(
          errors[0]?.message || "An error occurred during question creation"
        );
      }
  ...
  const updateAssignmentQuestion = async (
    variables: UpdateAssignmentQuestionVariables
  ) => {
    try {
      setLoading(true);
      setError(null);
      ...
      const { data, errors } =
        await apolloClient.mutate<UpdateAssignmentQuestionResponse>({
      ...
      if (errors) {
        console.error("GraphQL errors:", errors);
        throw new Error(
          errors[0]?.message || "An error occurred during question update"
        );
      }
  ...
  const deleteAssignmentQuestion = async (
    variables: DeleteAssignmentQuestionVariables
  ) => {
    try {
      setLoading(true);
      setError(null);
      ...
      const { data, errors } =
        await apolloClient.mutate<DeleteAssignmentQuestionResponse>({
      ...
      if (errors) {
        console.error("GraphQL errors:", errors);
        throw new Error(
          errors[0]?.message || "An error occurred during question deletion"
        );
      }
  ...
  const createAssignmentQuestionSolution = async (
    ...
    try {
      ...
      const { data, errors } =
        await apolloClient.mutate<CreateAssignmentQuestionSolutionResponse>({
      ...
      if (errors) {
        console.error("GraphQL errors:", errors);
        throw new Error(
          errors[0]?.message || "An error occurred while creating the solution"
        );
      }
  ...
  const updateAssignmentQuestionSolution = async (
    ...
    try {
      ...
      const { data, errors } =
        await apolloClient.mutate<UpdateAssignmentQuestionSolutionResponse>({
      ...
      if (errors) {
        console.error("GraphQL errors:", errors);
        throw new Error(
          errors[0]?.message || "An error occurred while updating the solution"
        );
      }
