...
import { Lecture, SourceCodeFile, VideoContent, ExternalResourceItem } from "@/lib/types";
import {
  ChevronDown,
  ChevronUp,
  Play,
...
// Define interface for content items in the sidebar
...
const StudentVideoPreview = ({ 
  ...
  const [sections, setSections] = useState([
    {
      ...
      lectures: [
        { 
          ...
          name: "vyioklcleaszc", 
          contentType: "video",
          description: "This is my description | just added.",
          resources: {
            downloadableFiles: [{ name: "Netflix.mp4", size: "1:45" }],
            sourceCodeFiles: [],
            externalResources: []
          }
        },
        { 
          ...
          resources: {
            downloadableFiles: [],
            sourceCodeFiles: [{ name: "find_max.py", type: "SourceCode" }],
            externalResources: [{ title: "External Resource Title", url: "https://example.com" }]
          }
        }
      ],
      assignments: [
  ...
  // Prepare sidebar content items for display
  const [sidebarItems, setSidebarItems] = useState<{[sectionId: string]: SidebarContentItem[]}>({});
  const [expandedSections, setExpandedSections] = useState<Record<string, boolean>>({
    section1: true // Initialize with the first section expanded
  });

  // Process sections into sidebar items
  useEffect(() => {
    const processedItems: {[sectionId: string]: SidebarContentItem[]} = {};
    
    sections.forEach(section => {
      const sectionItems: SidebarContentItem[] = [];
      
      // Process lectures
      section.lectures.forEach((lec, index) => {
        const itemType: ContentItemType = lec.contentType === 'article' ? 'article' : 'video';
        const hasResources = 
          (lec.resources?.downloadableFiles && lec.resources.downloadableFiles.length > 0) || 
          (lec.resources?.sourceCodeFiles && lec.resources.sourceCodeFiles.length > 0) ||
          (lec.resources?.externalResources && lec.resources.externalResources.length > 0);
        
        sectionItems.push({
          id: lec.id,
          name: lec.name,
          type: itemType,
          duration: lec.contentType === 'video' ? '1min' : '2min',
          isCompleted: false,
          hasResources: hasResources,
          resources: {
            downloadableFiles: lec.resources?.downloadableFiles || [],
            sourceCodeFiles: lec.resources?.sourceCodeFiles || [],
            externalResources: lec.resources?.externalResources || []
          }
        });
      });
      ...
      processedItems[section.id] = sectionItems;
    });
  ...
  const handleSelectItem = (itemId: string) => {
    ...
    // For now, we just close the preview if it's not the current lecture
    if (itemId !== lecture.id) {
  ...
  const ResourcesDropdown: React.FC<{
    ...
    const hasResources = 
      (resources.downloadableFiles && resources.downloadableFiles.length > 0) || 
      (resources.sourceCodeFiles && resources.sourceCodeFiles.length > 0) || 
      (resources.externalResources && resources.externalResources.length > 0);
      ...
      <div className="relative">
        ...
        {isOpen && (
          <div className="absolute right-0 mt-1 w-64 bg-white border border-gray-200 rounded-md shadow-lg z-10">
            <div className="p-2">
              {resources.downloadableFiles && resources.downloadableFiles.map((file, index) => (
              ...
              {resources.sourceCodeFiles && resources.sourceCodeFiles.map((file, index) => (
              ...
              {resources.externalResources && resources.externalResources.map((resource, index) => (
            ...
            {activeTab === 'overview' && !showSearch && (
              <div className="p-6">
                ...
                <div className="mb-8 pb-8 border-b border-gray-200 grid grid-cols-3">
                  ...
                  <div>
                    <p className="text-sm text-gray-700">Lectures: {sections.reduce((total, section) => total + section.lectures.length, 0)}</p>
            ...
            {sections.map(section => (
              <div key={section.id}>
                ...
                <div className="p-4 border-b border-gray-200">
                  ...
                  {/* Section stats: completed/total items | total duration */}
                  <p className="text-sm text-gray-500">
                    0/{sidebarItems[section.id]?.length || 0} | {sidebarItems[section.id]?.length || 0}min
                  </p>
                </div>
                
                {/* Section content items */}
                {expandedSections[section.id] && sidebarItems[section.id]?.map((item, index) => (
                  <div 
                    key={item.id} 
                    className={`p-4 ${item.id === lecture.id ? 'bg-gray-100 border-l-4 border-purple-600' : 'hover:bg-gray-50'} cursor-pointer`}
