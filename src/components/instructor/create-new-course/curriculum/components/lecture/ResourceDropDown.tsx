/*
 * STEP 2: Create ResourcesDropdown Component
 * This component displays a dropdown with resources (downloads, source code, external links)
 */
...
const ResourcesDropdown: React.FC<ResourcesDropdownProps> = ({ resources, isOpen, toggleOpen }) => {
  ...
  return (
    <div className="relative">
      ...
      >
        <span>Resources</span>
        {isOpen ? <ChevronDown className="w-4 h-4 ml-1 transform rotate-180" /> : <ChevronDown className="w-4 h-4 ml-1" />}
      </button>
          ...
          <div className="p-2">
            ...
            {resources.externalResources.map((resource, index) => (
              <div key={`er-${index}`} className="flex items-center py-1 px-2 hover:bg-gray-50">
                ...
                <a href={resource.url} className="text-sm text-blue-600 hover:underline" target="_blank" rel="noopener noreferrer">
                  {resource.title}
                </a>
              </div>
            ))}
